...
class Store:
	...
	def browse(self, limit=20, start=0):
		...
		return {'items':items,'total':cursor.count(),'url_prefix': config.url_prefix}
		
	def store(self, file, name):#ctype
...
#		data = file.read()
		data = file
		if (len(data) > int(config.max_file_size)):
		...
		return [True, fs.put(data, _id=id, filename=filename, type='image/'+ext,note=name), filename]
	...
	def getFs(self):
		...
		if self.db is None:
			self.db = self.getDb()
		return gridfs.GridFS(self.db,config.fs_prefix)

	def getDb(self):
		from pymongo import Connection
		c = Connection(config.servers)
		return c[config.db_name]

	def getCollection(self):
		if self.db is None:
			self.db = self.getDb()
		cn = '{0}.files'.format(config.fs_prefix)
		return self.db[cn]
